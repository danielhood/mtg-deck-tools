...
**Parallel track:** **UX2** wizard controls for `strict_dependencies`, `repair_dependencies`, and `mechanic_focus` ([11](11-dependency-engine-user-experience.md)) — does not block pattern work but improves user-facing control. Scope expanded 2026-06-03: focus-level presets (incidental/supported/focused/engine) now target **all 15 profiles activated by user selections** (not only energy and auras). No engine or schema changes required; `DeckCriteria.mechanic_focus` and `dependency_scope.py` already support every profile ID.
