...
**Dependency expansion:** **tutor payload matching** (CMC bands, colors, land subtypes, multi-type OR for `TUTOR_TARGET_EXISTS`), **enchantments** profile (`ENCHANTMENT_SUPPORT_MIN`, wizard `themes: [enchantress]`), **tokens** (`TOKEN_BALANCE`), **vehicles** (`VEHICLE_BALANCE`), and **subtype lords** — shipped as of 2026-06. Next: graveyard/landfall heuristics, UX2 wizard dependency controls — see [`planning/09-next-steps.md`](planning/09-next-steps.md).
