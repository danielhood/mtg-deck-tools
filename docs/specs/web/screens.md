...
| Import deck | **UX13b** shipped — paste, **Download template**, or **Choose text file**; **UX13c** active — **Preview** (`POST /api/v1/decks/import/preview`) → line table → gated **Import** (`POST /api/v1/decks/import`) → `/deck/:id` when DB ready |
...
| Filters | Chip groups: **Slot** (multi), **Type** (multi — Creature, Vehicle, Equipment, Instant, …), **Color** (multi — only WUBRG/void pips present in the deck). **AND across groups** (slot + type + color). **Color:** every selected pip must appear on the card (e.g. Green + Red → only cards with both); nonlands use casting-cost pips, lands use produced mana / type line / identity — see [user-experience.md](../dependency-engine/user-experience.md) § UX7e. Empty state when filter matches nothing |
| Filtered count | Below filters — quantity-weighted card total for the current filter (`N cards`, or `N of M cards` when any filter is active); updates live as filters change |
...
| Import | **Home only** (**UX13b**); library import deferred |
...
## Advanced swap (`/deck/:id` sheet) — UX12
...
| Region | Behavior |
| --- | --- |
| Entry | **Advanced…** on swap bar; **Fix issue…** on dependency issue row; named-card mode within sheet |
| Sheet | Bottom overlay on `/deck/:id`; strategy presets + filters + optional preview |
| Quick fix | Prototype tertiary on issue row — apply default strategy with confirm; evaluate after dogfood |
