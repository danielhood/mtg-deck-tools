...
- **`commanders/search`** — query: `q`, `colors`, `color_match=includes|exact` (SPA default **exact**; API default `includes` when omitted), `card_price_min_usd` / `card_price_max_usd` from **commander step** bounds (min defaults from budget per-card minimum; independent of deck build filters), optional `budget_usd` / `strict_budget`.
...
| POST | `/api/v1/import` | Oracle import + optional Scryfall download; wired from SPA home (`DbBanner`, refresh on `HomePage`) |
